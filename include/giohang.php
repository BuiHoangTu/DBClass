<?php
 if(isset($_POST['themgiohang'])){
	...
	if($_POST['product_id'] == NULL) {
		$tmp = 0;
	} else $tmp = count($_POST['product_id']);
 	for($i=0;$i<$tmp;$i++){
			...
			<div class="checkout-right">
				<div class="table-responsive">
					...
					<table class="timetable_sub">
						...
						$sql_sanpham = mysqli_query($con,"SELECT * FROM tbl_sanpham WHERE sanpham_id='$sanpham_id'");
						$row_fetch_sanpham = mysqli_fetch_array($sql_sanpham);
						$i = 0;
						$total = 0;
						...
						?>
							<tr class="rem1">
								...
								<td class="invert-image" height="120px">
									<a href="single.html">
										<img src="images/<?php echo $row_fetch_giohang['hinhanh'] ?>" alt=" " class="img-responsive" style="object-fit: cover;">
									</a>
								</td>
								<td class="invert">
									<input type="hidden" name="product_id[]" value="<?php echo $row_fetch_giohang['sanpham_id'] ?>">
									<input type="number" min="1" max="<?php echo number_format($row_fetch_sanpham['sanpham_soluong']) ?>" name="soluong[]" value="<?php echo $row_fetch_giohang['soluong'] ?>">
									<?php
										$sanpham_id = $row_fetch_giohang['sanpham_id'];
										$soluong = $row_fetch_giohang['soluong'];
										$sql_reset = mysqli_query($con,"UPDATE tbl_sanpham SET sanpham_soluong=sanpham_soluong+'$soluong' WHERE sanpham_id='$sanpham_id'");
									?>
								</td>
							...
							<tr>
								...
								<?php 
										$sanpham_id = $row_1['sanpham_id'];
										$soluong = $row_1['soluong'];
										$sql_thanhtoan = mysqli_query($con,"UPDATE tbl_sanpham SET sanpham_soluong=sanpham_soluong-'$soluong' WHERE sanpham_id='$sanpham_id'");
								}
								?>
					...
					<form action="" method="post" class="creditly-card-form agileinfo_form">
						<div class="creditly-wrapper wthree, w3_agileits_wrapper">
							<div class="information-wrapper">
								...
								<?php
									$sanpham_id = $row_thanhtoan['sanpham_id'];
									$soluong = $row_thanhtoan['soluong'];
									$sql_thanhtoan = mysqli_query($con,"UPDATE tbl_sanpham SET sanpham_soluong=sanpham_soluong-'$soluong' WHERE sanpham_id='$sanpham_id'");
								} 
								?>
